...
class Encoder(nn.Module):
    ...
    def forward(self, x_):
        ...
        return x, skip1, skip2, skip3, input1, input2, input3
...
class Decoder(nn.Module):
    ...
    def __init__(self, dec_in, dec_out, n_dim, leaky_relu_alpha=0.3):
        ...
        self.conv2d_transpose_1 = nn.ConvTranspose2d(dec_in + n_dim + 2, n_dim, kernel_size=(3,3), stride=1, padding=1)
        ...
        self.conv2d_transpose_4 = nn.ConvTranspose2d(2 * n_dim + 2, n_dim, kernel_size=(3,3), stride=1, padding=1)
        ...
        self.conv2d_transpose_7 = nn.ConvTranspose2d(2 * n_dim + 2, n_dim, kernel_size=(3,3), stride=1, padding=1)
    ...
    def forward(self, x, skip1, skip2, skip3, input1, input2, input3):
        x = self.leaky_relu(self.conv2d_transpose(x))
        x = self.up_sampling2d(x)
        x = torch.cat((x, skip3, input3), dim=1)
        ...
        x = torch.cat((x, skip2, input2), dim=1)
        ...
        x = torch.cat((x, skip1, input1), dim=1)
