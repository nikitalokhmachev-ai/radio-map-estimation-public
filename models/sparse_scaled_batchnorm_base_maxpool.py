...
from .sparse_scaled_batchnorm import SparseScaledBNConv2d

device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

class Encoder(nn.Module):
    def __init__(self, enc_in, enc_out, n_dim, scale=9, leaky_relu_alpha=0.3):
        super(Encoder, self).__init__()
        self.conv2d = SparseScaledBNConv2d(enc_in, n_dim, kernel_size=(3, 3), padding='same', scale=scale)
        self.conv2d_1 = SparseScaledBNConv2d(n_dim, n_dim, kernel_size=(3, 3), padding='same', scale=scale)
        self.conv2d_2 = SparseScaledBNConv2d(n_dim, n_dim, kernel_size=(3, 3), padding='same', scale=scale)
        self.conv2d_3 = SparseScaledBNConv2d(n_dim, n_dim, kernel_size=(3, 3), padding='same', scale=scale)
        self.conv2d_4 = SparseScaledBNConv2d(n_dim, n_dim, kernel_size=(3, 3), padding='same', scale=scale)
        self.conv2d_5 = SparseScaledBNConv2d(n_dim, n_dim, kernel_size=(3, 3), padding='same', scale=scale)
        self.conv2d_6 = SparseScaledBNConv2d(n_dim, n_dim, kernel_size=(3, 3), padding='same', scale=scale)
        self.conv2d_7 = SparseScaledBNConv2d(n_dim, n_dim, kernel_size=(3, 3), padding='same', scale=scale)
        self.conv2d_8 = SparseScaledBNConv2d(n_dim, n_dim, kernel_size=(3, 3), padding='same', scale=scale)
        self.mu = SparseScaledBNConv2d(n_dim, enc_out, kernel_size=(3, 3), padding='same', scale=scale)

        self.average_pooling2d = nn.AvgPool2d(kernel_size=(2, 2))
        self.mask_max_pooling2d = nn.MaxPool2d(kernel_size=(2, 2))
        self.mask_max_pooling2d_pad = nn.MaxPool2d(kernel_size=(3,3), padding=1, stride=1)
    ...
    def forward(self, x, mask):
        ...
        x = self.leaky_relu(x)
        mask = self.mask_max_pooling2d_pad(mask)
        ...
        mask = self.mask_max_pooling2d(mask)
