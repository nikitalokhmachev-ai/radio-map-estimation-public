...
class Encoder(nn.Module):
    def __init__(self, enc_in, enc_out, n_dim, n_dim_mask, leaky_relu_alpha=0.3):
        super(Encoder, self).__init__()
        self.n_dim_mask = n_dim_mask
        if self.n_dim_mask is None:
            raise ValueError('Must set integer value >= 0 for number of filters to convolve mask (n_dim_mask)')

        self.conv2d = nn.Conv2d(enc_in, n_dim, kernel_size=(3, 3), padding='same')
        self.conv2d_1 = nn.Conv2d(n_dim + n_dim_mask + 1, n_dim, kernel_size=(3, 3), padding='same')
        self.conv2d_2 = nn.Conv2d(n_dim + n_dim_mask + 1, n_dim, kernel_size=(3, 3), padding='same')
        self.average_pooling2d = nn.AvgPool2d(kernel_size=(2, 2))
        self.conv2d_3 = nn.Conv2d(n_dim + n_dim_mask + 1, n_dim, kernel_size=(3, 3), padding='same')
        self.conv2d_4 = nn.Conv2d(n_dim + n_dim_mask + 1, n_dim, kernel_size=(3, 3), padding='same')
        self.conv2d_5 = nn.Conv2d(n_dim + n_dim_mask + 1, n_dim, kernel_size=(3, 3), padding='same')
        self.average_pooling2d_1 = nn.AvgPool2d(kernel_size=(2, 2))
        self.conv2d_6 = nn.Conv2d(n_dim + n_dim_mask + 1, n_dim, kernel_size=(3, 3), padding='same')
