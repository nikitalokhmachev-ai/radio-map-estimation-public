...
class GudmundsonMapGenerator(MapGenerator):
    ...
    Only one of the following can be set:
        ...
        # EDIT: changed to reflect order of array axes
        tx_power: num_bases x num_sources matrix with the transmitter power.
    ...
    def __init__(self,
        ...
        # EDIT: Added if statement to assert that n_tx matches second axis (num_sources) in tx_power
        if self.tx_power is not None:
          assert self.n_tx == self.tx_power.shape[1]
    ...
    def generate_power_map_per_freq(self, num_bases):
        ...
        # EDIT: Original code, ``if self.tx_power_interval:``, raised errors
        if self.tx_power_interval is not None:
            tx_power_interval_nat = dbm_to_natural(np.array(self.tx_power_interval))
            n_sources = self.n_tx
        
        # EDIT: Original code, ``if self.tx_power.all():``, raised errors
    ...
    def generate_shadowing(self, size=(1, 1)):
        ...
        # EDIT: Added for loop expanding dimension of shadowing_dB for any axis with size of 1.
        for i in range(len(size)):
          if size[i] == 1:
            shadowing_dB = np.expand_dims(shadowing_dB, axis=i)
